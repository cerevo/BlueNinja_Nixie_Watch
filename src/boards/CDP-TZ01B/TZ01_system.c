...
bool TZ01_system_tick_start_us(TZ01_SYSTEM_TICK_NO tim_no, uint32_t us_timeout)
{
    uint32_t tick_curr;
    uint32_t prescale, tmr_clock;
    
    prescale = tz10xx_drv_pmu->GetPrescaler(PMU_CD_PPIER0);
    tmr_clock = 48 / prescale;   //Hz
    ...
    tick_curr = tz10xx_drv_tmr->GetValue();
    tz01_system_tick.timers[tim_no].timeout = tick_curr - (uint32_t)((us_timeout * tmr_clock) - 1);
    tz01_system_tick.timers[tim_no].is_active = true;
...
bool TZ01_system_init(void)
{
    ...
    tz10xx_drv_pmu->SelectClockSource(PMU_CSM_UART0, PMU_CLOCK_SOURCE_OSC12M);
    ...
    tz10xx_drv_pmu->SetPrescaler(PMU_CD_UART0, 1);
